...
export function xanoToLocal(xanoItem: XanoShoppingItem): ShoppingItem {
  return {
    ...
    preferredSource: xanoItem.source_url || '',
...
export function localToXano(localItem: ShoppingItem, userId: string = 'default'): CreateShoppingItemRequest {
  return {
    user: userId,
    name: localItem.item,
    description: '', // Could be derived from notes or left empty
    priority: localItem.priority.toLowerCase() as 'low' | 'medium' | 'high',
    source_url: localItem.preferredSource,
    status: mapLocalStatusToXano(localItem.status, localItem.completed),
